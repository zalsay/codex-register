...
SCP_TARGETS = _config.get("scp_targets", [])
SCP_HOST = _config.get("scp_host", "")
SCP_PATH = _config.get("scp_path", "")
SCP_PORT = _config.get("scp_port", "")
SCP_USER = _config.get("scp_user", "")

UPLOAD_API_URL = os.environ.get("UPLOAD_API_URL", UPLOAD_API_URL)
UPLOAD_API_TOKEN = os.environ.get("UPLOAD_API_TOKEN", UPLOAD_API_TOKEN)
PROXY = os.environ.get("PROXY", PROXY)
CF_WORKER_DOMAIN = os.environ.get("CF_WORKER_DOMAIN", CF_WORKER_DOMAIN)
CF_EMAIL_DOMAIN = os.environ.get("CF_EMAIL_DOMAIN", CF_EMAIL_DOMAIN)
CF_ADMIN_PASSWORD = os.environ.get("CF_ADMIN_PASSWORD", CF_ADMIN_PASSWORD)
SCP_TARGETS = os.environ.get("SCP_TARGETS", SCP_TARGETS)
SCP_HOST = os.environ.get("SCP_HOST", SCP_HOST)
SCP_PATH = os.environ.get("SCP_PATH", SCP_PATH)
SCP_PORT = os.environ.get("SCP_PORT", SCP_PORT)
SCP_USER = os.environ.get("SCP_USER", SCP_USER)
...
SCP_TARGETS = _parse_scp_targets({
...
def _upload_token_json_scp(filename: str, target: dict) -> None:
    """使用 scp 上传 Token JSON 文件"""
    try:
        target_str = _build_scp_target(filename, target)
        cmd = ["scp", "-q", "-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=accept-new"]
        port = str(target.get("port", "")).strip()
        if port:
            cmd += ["-P", port]
        cmd += [filename, target_str]
        subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        print(f"  ✅ Token JSON 已上传到 {target_str}")
...
def upload_token_json(filename):
    ...
    if SCP_TARGETS:
        for target in SCP_TARGETS:
            _upload_token_json_scp(filename, target)
        return
